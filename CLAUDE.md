...
**Claude Code의 커밋 및 푸시 정책:**

1. **커밋**: **사용자의 명시적 요청이 있을 때만** 커밋
   - "커밋해줘", "진행해줘", "commit" 같은 요청 필요
   - 코드 변경 후 사용자가 확인하고 승인할 때까지 대기

2. **푸시**: **사용자의 명시적 요청이 있을 때만** 원격 저장소에 푸시
   - "푸시해줘", "push 해줘" 같은 요청이 필요
   - 커밋 후 사용자 확인 후 요청 시에만 수행

**이유**: 
- 사용자가 모든 변경사항을 명확히 검토 후 진행
- 코드 리뷰 및 테스트 기회 제공
- 불필요한 커밋/푸시 방지
- 사용자의 완전한 제어 보장
